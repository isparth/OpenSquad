...
- Verification: jsdom tests with a fake `EventSource` (`test/fake-event-source.ts`) and stubbed fetch; a real Electron dev run against a local API booted with a scripted fake runtime on another port (create → send → stream → cancel → reload → remove key); then one manual live session against OpenAI Agents from the desktop. Provider `usage` came back null on live runs — check the adapter's turn usage mapping before relying on token counts.
