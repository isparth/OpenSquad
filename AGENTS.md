...
- OpenAI `command_execution` items normalize to assistant messages with a capped `command` content part; environment events map to `environment.status` and `runtime_sessions.environment_status`, while other non-message items remain ignored.
