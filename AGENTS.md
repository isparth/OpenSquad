...
- `CreateRuntimeSessionOptions.environment` defaults to `"hosted"` at contract level; admission always sets it explicitly. `input` is required and non-blank with `"none"`, and rejected for hosted sessions. Runtimes declare environmentless support with `features.environmentless`.
...
- Current scope: desktop bot management, agent CRUD/avatars, the runtime adapter, S2a's conversation/run backend, S2b's development-only credential vault, and S2c's desktop chat. Text-only chat with a bot works end to end in unpackaged development. Authenticated desktop identity, production credential storage, authenticated Composio MCP provisioning, tool/artifact display, memory, and skills/artifact transfer are not implemented yet.
...
- Migration `0001_sturdy_mercury.sql` adds conversation/run storage. Migration `0002_confused_wiccan.sql` adds per-bot sandbox preference and `runtime_sessions.environment`; its `hosted` DB default backfills legacy sessions and supports older checkouts, while admission always writes the selected environment explicitly. Run `pnpm db:migrate` before S2a tests. Focused checks: `pnpm --filter @opensquad/api exec vitest run test/conversations.test.ts test/run-admission.test.ts test/runtime-persistence.test.ts test/runs.test.ts test/conversation-events.test.ts test/runtime-safety.test.ts`.
...
- Verification: jsdom tests with a fake `EventSource` (`test/fake-event-source.ts`) and stubbed fetch; a real Electron dev run against a local API booted with a scripted fake runtime on another port (create → send → stream → cancel → reload → remove key); then one manual live session against OpenAI Agents from the desktop. Provider `usage` came back null on those live runs: terminal turn events carry a top-level `usage` for the root agent while the embedded `turn.usage` is best-effort, so the adapter now prefers the event-level value (PR #6). Confirm non-null `usage` on the next live turn before relying on token counts.
